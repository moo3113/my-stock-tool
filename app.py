...
if st.button("🚀 執行 AI 數據分析儀"):
    try:
        ...
        if p_val:
            ...
            delay_warn = ""
            if trading:
                delay_warn = '''<div style="background: #e3f2fd; color: #1565c0; padding: 12px; border-radius: 12px; font-size: 11px; text-align: center; margin-bottom: 20px; border: 1px solid #bbdefb; font-weight: bold;">⚠️ 交易時間數據約有 15-20 分鐘延遲，建議盤後試算或配合券商 App 報價。</div>'''
            ...
            sl_html = f'''<tr style="background: #fff0f0; border: 1.5px dashed #ff8787;"><td style="padding: 10px 5px; color: #cc0000; font-weight: bold;">🚩 風控回檔位 (-6%)<br><span style="font-size:10px; color:#999;">*突破點回檔之簡化風控</span></td><td style="text-align: right; font-weight: bold; color: #cc0000;">{round(p104*0.94,2):.2f}</td></tr>''' if p_val >= p104 else ""
            ...
            <div style="font-family: sans-serif; background: white; padding: 15px; border-radius: 25px; color: #333;">
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 16px; margin-bottom: 25px;">
                    ...
                    <tr style="background: #fff9db;"><td style="padding: 10px 5px; color: #e67e22; font-weight: bold;">突破點 (1.04)<br><span style="font-size:10px; color:#666;">*空頭格局通常在此位站不穩</span></td><td style="text-align: right; font-weight: bold; color: #e67e22;">{p104:.2f}</td></tr>
                    {sl_html}
                    ...
                    <tr style="border-bottom: 1px solid #eee;"><td style="padding: 10px 5px; font-weight: bold;">目標三 (1.7) <span style="font-size:11px; color:#cc0000;">*高風險不追價</span></td><td style="text-align: right;">{t3:.2f}</td></tr>
                </table>
                <div style="background: #fff9c4; padding: 20px; border-radius: 20px; border: 3px solid #fbc02d;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr; gap: 10px;">
                        <div style="background: white; padding: 12px; border-radius: 12px; border-left: 5px solid #fbc02d; display: flex; justify-content: space-between;"><span>1. <b>強勢股判定</b>：站穩季線之上</span> <span>{"✅" if p_val > m60 else "❌"}</span></div>
                        <div style="background: white; padding: 12px; border-radius: 12px; border-left: 5px solid #fbc02d; display: flex; justify-content: space-between;"><span>2. <b>趨勢判定</b>：多頭型態 (價>月>季)</span> <span>{"✅" if p_val > m20 > m60 else "❌"}</span></div>
                        <div style="background: white; padding: 12px; border-radius: 12px; border-left: 5px solid #fbc02d; display: flex; justify-content: space-between;"><span>3. <b>籌碼判定</b>：主力連買</span> <span>✅</span></div>
                        <div style="background: white; padding: 12px; border-radius: 12px; border-left: 5px solid #fbc02d; display: flex; justify-content: space-between;"><span>4. <b>攻擊判定</b>：突破成本 1.04 空間</span> <span>{"✅" if p_val >= p104 else "❌"}</span></div>
                    </div>
                </div>
            </div>
            '''
            components.html(full_card, height=2100, scrolling=True)
            st.plotly_chart(go.Figure(data=[go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], increasing_line_color='#ff4d4d', decreasing_line_color='#00b050')]).update_layout(xaxis_rangeslider_visible=False, template="plotly_dark", height=400), use_container_width=True)
