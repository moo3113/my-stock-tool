...
def get_data(sid):
    ...
    for suffix in [".TW", ".TWO"]:
        try:
            ...
            df = ticker.history(period="120d") # 抓長一點確保 60MA 算得出來
        ...
        with st.spinner('金兔正在同步大數據...'):
            ...
            if price is not None and not pd.isna(price):
                ...
                if price < cost: 
                    ...
                    strategy, color = f"📍 強勢波段行進中，注意 {t2} 關卡壓力。", "#ff6b6b"

                # 神獸警告
                god_beast_html = f'''
                <div style="background: linear-gradient(135deg, #FFD700, #DAA520); color: #3d2b1f; padding: 15px; margin: 15px; border-radius: 12px; text-align: center; font-weight: bold; border: 1px solid #B8860B;">
                    ✨ 偵測到「上古神獸」✨<br>成本過高（已達現價1.7倍），建議改用【融資成本】。
                </div>
                ''' if is_god_beast else ""

                # AI 趨勢分析
                trend_text = "📈 趨勢偏多，股價在季線之上。" if price > ma60 else "📉 趨勢偏弱，股價在季線之下。"
                bias = ((price - ma60) / ma60) * 100
                bias_note = f"<br>🚨 正乖離率過大 ({bias:.1f}%)，注意修正風險。" if bias > 15 else ""
                ...
                <div style="font-family: sans-serif; background: #fffafb; padding: 5px;">
                    <div style="background: white; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); border: 1px solid #ffe3e3;">
                        ...
                        <div style="background: #f0f7ff; border-left: 5px solid #228be6; padding: 15px; margin: 0 20px 20px 20px; border-radius: 8px;">
                            ...
                            <div style="font-size: 14px; line-height: 1.5;">{trend_text}{bias_note}</div>
                        </div>
                        ...
                        <div style="background: #ebfbee; padding: 20px; border-top: 2px dashed #b2f2bb;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                <div style="font-size: 12px; color: #2b8a3e; background: white; padding: 8px; border-radius: 8px; border: 1px solid #d3f9d8;">🚀 <b>優先強勢股</b><br>選有動能的標的</div>
                                <div style="font-size: 12px; color: #2b8a3e; background: white; padding: 8px; border-radius: 8px; border: 1px solid #d3f9d8;">🤝 <b>法人持續加碼</b><br>籌碼面有支撐</div>
                                <div style="font-size: 12px; color: #2b8a3e; background: white; padding: 8px; border-radius: 8px; border: 1px solid #d3f9d8;">📉 <b>確認多頭線型</b><br>趨勢向上不買弱</div>
                                <div style="font-size: 12px; color: #2b8a3e; background: white; padding: 8px; border-radius: 8px; border: 1px solid #d3f9d8;">🎯 <b>現價離成本近</b><br>回檔佈局最安全</div>
                            </div>
                        </div>
                    </div>
                </div>
