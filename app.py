...
def fetch_smart_data(sid):
    ...
    # 判斷是否為交易時間 (09:00-13:30 且為週一至週五)
    is_trading_hours = ("09:00" <= now_t <= "13:30") and (now.weekday() < 5)
    # 判斷試搓時段
    ...
    for suffix in [".TW", ".TWO", ""]:
        try:
            ...
            if not df.empty:
                ...
                return name, df_clean, p, ch, is_trial, is_trading_hours
        except: continue
    return None, None, None, 0, False, False

if st.button("🚀 執行 AI 數據分析儀"):
    try:
        cost = float(cost_str); name, df, p_val, ch_val, trial, trading = fetch_smart_data(sid_str)
        if p_val:
            ...
            ma60_note = f'<div style="font-size:10px; color:#e67e22; margin-top:4px;">*掛牌僅{len(df)}天,以至今平均計算</div>' if len(df) < 60 else ""
            
            # 🕒 只有「交易時間」才顯示數據延遲提醒
            delay_warn = ""
            if trading:
                delay_warn = '''
                <div style="background: #fff4e5; color: #663c00; padding: 10px; border-radius: 12px; font-size: 11px; text-align: center; margin-bottom: 20px; border: 1px solid #ffe2b7; font-weight: bold;">
                    ⚠️ 目前為交易時段，數據約有 15-20 分鐘延遲。<br>現價判定僅供參考，請配合券商 App 獲取即時報價。
                </div>
                '''
            ...
            <div style="font-family: sans-serif; background: white; padding: 15px; border-radius: 25px; color: #333;">
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 16px; margin-bottom: 25px;">
                    ...
                    <tr style="background: #fff9db;"><td style="padding: 10px 5px; color: #e67e22; font-weight: bold;">突破點 (1.04)<br><span style="font-size:10px; color:#666;">*空頭格局在此位站不穩</span></td><td style="text-align: right; font-weight: bold;">{p104:.2f}</td></tr>
                    {sl_html}
