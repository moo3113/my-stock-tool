...
# 1. 基礎設定
st.set_page_config(page_title="仙兔 AI 全資產分析儀", page_icon="🐰", layout="centered")
...
# 2. 原生 HTML 九宮格輸入組件 (上下排列，完美間距)
query_params = st.query_params
sid_str = query_params.get("sid", "0050") # 預設改為 0050
cost_str = query_params.get("cost", "180.00")
...
html_input_component = f"""
    <div style="font-family: -apple-system, sans-serif; padding: 10px 5px;">
        ...
        <div style="margin-bottom: 15px;">
            <div style="color: #eee; text-align: center; font-weight: bold; margin-bottom: 12px; font-size: 16px;">買入/發行成本</div>
...
def get_stock_data_universal(sid):
    sid = sid.strip()
    # 嘗試多種台灣股市常見字尾
    for suffix in [".TW", ".TWO", ""]:
        try:
            ...
            if not df.empty and len(df) >= 2:
                # 抓取名稱優先權：Yahoo info > twstock
                try:
                    name = ticker.info.get('shortName', ticker.info.get('longName'))
                    if not name or name == sid:
                        name = twstock.codes.get(sid).name
                except:
                    name = f"市值型資產 {sid}"
...
if st.button("🚀 執行 AI 全資產大分析"):
    try:
        name, df, price, change = get_stock_data_universal(sid_str)
        cost = float(cost_str)
        ...
        if price:
            ...
            # 格局判定
            if price > m20 > m60: status = ("🔥 強勢多頭", "#fff9db", "【迎向暴漲】慣性向上，回踩不破季線即是機會。")
            elif price < m20 < m60: status = ("❄️ 弱勢空頭", "#e9ecef", "【防範暴跌】下方支撐弱，1.04 突破點站不穩。")
            else: status = ("🌀 盤整格局", "#e3fafc", "【50% 盤整危險區】別碰！等待方向明確。")
            ...
            # 視覺智慧建議
            ai_vis = get_visual_advise(df)
            vw_html = "".join([f'<div style="margin-bottom:6px;">{w}</div>' for w in ai_vis])
            ...
            <div style="font-family: sans-serif; background: white; padding: 15px; border-radius: 25px; color: #333;">
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 16px; margin-bottom: 25px;">
                    <tr><td style="padding: 10px 5px;">買入/法人成本</td><td style="text-align: right; font-weight: bold;">{cost:.2f}</td></tr>
                    ...
                    <tr style="border-bottom: 1px solid #eee;"><td style="padding: 10px 5px; font-weight: bold;">{lt3}目標三 (1.7)</td><td style="text-align: right; {st3}">{t3:.2f}</td></tr>
                </table>
                ...
                <div style="background: #fff9c4; padding: 20px; border-radius: 20px; border: 3px solid #fbc02d;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr; gap: 10px;">
                        ...
                        <div style="background: white; padding: 12px; border-radius: 12px; border-left: 5px solid #fbc02d; display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 14px;">3. <b>突破攻擊</b>：突破 1.04 點</span> <span style="font-size: 20px;">{"✅" if price >= p104 else "❌"}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        else: st.error("❌ 找不到數據，請確認代號是否正確（例如：0050, 006208, 009816）。")
