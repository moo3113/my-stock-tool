...
def get_google_finance_price(sid):
    """從 Google Finance 抓取價格，穩定性較高"""
    # 嘗試上市 (TPE) 與 上櫃 (TWO)
    for exchange in ["TPE", "TWO"]:
        try:
            url = f"https://www.google.com/finance/quote/{sid}:{exchange}"
            headers = {'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X)'}
            resp = requests.get(url, headers=headers, timeout=5)
            if resp.status_code == 200:
                soup = BeautifulSoup(resp.text, 'html.parser')
                # Google Finance 的價格標籤通常包含 data-last-price
                price_tag = soup.select_one('div[data-last-price]')
                if price_tag:
                    return float(price_tag['data-last-price'])
                
                # 備案：尋找特定的 Class
                price_element = soup.find("div", {"class": "YMlS7e"})
                if price_element:
...
def get_stock_name(sid):
    ...
    try:
        ...
        return stock_info.name if stock_info else sid
    except:
        return sid

if st.button("🚀 開始計算報告"):
    with st.spinner('正在從 Google Finance 同步數據...'):
        stock_name = get_stock_name(sid)
        current_price = get_google_finance_price(sid)
        ...
        if current_price:
            ...
            st.error(f"❌ 依然抓不到 {sid} 的價格。這可能是伺服器連線受限，請嘗試在幾分鐘後重新點擊。")

st.caption("數據來源：twstock & Google Finance | 邏輯：仙兔關卡價")
