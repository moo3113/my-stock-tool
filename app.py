...
.stButton>button { width: 100%; border-radius: 15px; height: 3.5em; background: linear-gradient(135deg, #ff6b6b, #f06595); color: white; font-weight: bold; border: none; }</style>""", unsafe_allow_html=True)
...
def get_data(sid):
    ...
    for suffix in [".TW", ".TWO"]:
        try:
            ...
            # 增加抓取天數到 150 天，確保計算 60MA 不會因假日出錯
    ...
    # C. 如果 yfinance 還是失敗，嘗試 twstock 即時報價備援 (僅現價)
...
if st.button("🚀 執行 AI 數據分析"):
    try:
        cost = float(cost_str)
        with st.spinner('金兔正在從雲端搬運數據...'):
            name, price, ma60 = get_data(sid)

            if price is not None and not pd.isna(price) and ma60 is not None:
                # --- 數據計算 ---
