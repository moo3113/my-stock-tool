...
def get_visual_advise(df):
    ...
    if std < (last['Close'] * 0.015): advise.append("📦 【50% 盤整區】危險別碰！等待方向。")
    return advise
    ...
    for suffix in [".TW", ".TWO", ""]:
        try:
            ...
            if not df.empty:
                ...
                p = float(df['Close'].iloc[-1]); ch = p - float(df['Close'].iloc[-2])
                return name, df, p, ch
        except: continue
    return None, None, None, 0

if st.button("🚀 執行 AI 終極分析儀"):
    try:
        ...
        if price:
            ...
            m20_val = df['MA20'].iloc[-1]; m60_val = df['MA60'].iloc[-1]
            
            # 格局判定
            if price > m20_val > m60_val: status = ("🔥 強勢多頭", "#fff9db", "符合「迎向暴漲」格局。")
            elif price < m20_val < m60_val: status = ("❄️ 弱勢空頭", "#e9ecef", "防範暴跌，突破點站不穩。")
            else: status = ("🌀 箱型盤整", "#e3fafc", "50% 盤整區，別碰。")
            
            is_god = price >= (cost * 1.7)
            god_html = f'''<div style="background: #ff4b4b; color: white; padding: 15px; margin-bottom: 15px; border-radius: 15px; text-align: center; font-weight: bold; border: 3px solid white;">⚠️ 偵到「上古神獸」格局<br><span style="font-size:13px; font-weight:normal;">現價已超過成本 1.7 倍，建議改用「融資成本」重新計算分析。</span></div>''' if is_god else ""
            ...
            # --- 🛡️ 動態風控邏輯：進入 1.04 以上，顯示 -6% 風控參考 ---
            stop_loss_val = round(p104 * 0.94, 2)
            sl_html = f'''<tr style="background: #fff0f0; border: 1.5px dashed #ff8787;"><td style="padding: 10px 5px; color: #cc0000; font-weight: bold;">🚩 風控回檔位 (-6%)<br><span style="font-size:10px; color:#999;">*突破點回檔之簡化風控</span></td><td style="text-align: right; font-weight: bold; color: #cc0000;">{stop_loss_val:.2f}</td></tr>''' if price >= p104 else ""
            ...
            <div style="font-family: sans-serif; background: white; padding: 15px; border-radius: 25px; color: #333;">
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 16px; margin-bottom: 25px;">
                    ...
                    <tr style="background: #fff9db;"><td style="padding: 10px 5px; color: #e67e22; font-weight: bold;">{l104}突破點 (1.04)<br><span style="font-size:10px; color:#666; font-weight:normal;">*空頭格局通常在此位站不穩</span></td><td style="text-align: right; {s104}">{p104:.2f}</td></tr>
                    {sl_html}
                ...
                <div style="background: #fff9c4; padding: 20px; border-radius: 20px; border: 3px solid #fbc02d;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr; gap: 10px;">
                        <div style="background: white; padding: 12px; border-radius: 12px; border-left: 5px solid #fbc02d; display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 14px;">1. <b>強勢股判定</b>：站穩季線之上</span> <span style="font-size: 20px;">{"✅" if price > m60_val else "❌"}</span>
                        </div>
                        <div style="background: white; padding: 12px; border-radius: 12px; border-left: 5px solid #fbc02d; display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 14px;">2. <b>趨勢判定</b>：多頭型態 (價>月>季)</span> <span style="font-size: 20px;">{"✅" if price > m20_val > m60_val else "❌"}</span>
                        </div>
                        <div style="background: white; padding: 12px; border-radius: 12px; border-left: 5px solid #fbc02d; display: flex; justify-content: space-between; align-items: center;">
